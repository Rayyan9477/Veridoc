"""Injection-harness reporting (promoted from ``tests/eval/inject``).
...
from src.evaluation.inject.runner import InjectionResult, InjectionType
...
class InjectionReport:
    ...
    def overall_catch_rate(self, layer: str) -> float:
        """Catch rate for a layer across all injection types combined."""
        caught = missed = 0
        for inj_type in self.matrix.get(layer, {}):
            cell = self.matrix[layer][inj_type]
            caught += cell.get(CAUGHT, 0)
            missed += cell.get(MISSED, 0)
        total = caught + missed
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "overall_catch_rate": {
                layer: self.overall_catch_rate(layer) for layer in self.matrix
            },
